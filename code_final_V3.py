...
cote = str(input("Quel coté : "))
c = rsk.Client(host='192.168.0.104',key='')
...
if cote == 'g':
    ...
    def positionnement1(y1,cos, y_Axe):
        if y1 >= c.ball[1]:
            #c.blue1.goto((c.ball[0] + 0.2, y_Axe, math.pi - cos), wait = False)  # le robot se positionne sur la droite (y)
            c.blue1.goto((c.ball[0]+0.05, c.ball[1]-0.03, math.pi - cos), wait = False)  # le robot avance a la balle
        else:
            #c.blue1.goto((c.ball[0] + 0.2, y_Axe, math.pi + cos), wait = False)  # le robot se positionne sur la droite (y)
            c.blue1.goto((c.ball[0]+0.05, c.ball[1]+0.03, math.pi + cos), wait = False) #le robot avance a la balle
    ...
    def gardien():
        ...
        y_gardien()
        if -0.3<= y_goal <= 0.3:
            c.blue2.goto((0.8, y_goal, math.pi), wait = False)
        #if y_goal>0:
         #   c.blue2.goto((0.8, yb/2, math.pi))
        if abs(xb2 - xb) <= 0.15 and abs(yb2 - yb) <= 0.15:
            kick2(t_ref)
    ...
    def recule(y1,cos):
        if y1>=c.ball[1] :
            c.blue1.goto((c.blue1.position[0]-0.2,c.blue1.position[1], math.pi-cos), wait = False) #!!!!!!!!!!!!!!!!!!!! + -> -   #fait reculer le robot pout le sortir de la zone rouge autour de la balle
        else :
            c.blue1.goto((c.blue1.position[0]-0.2 ,c.blue1.position[1], math.pi+cos), wait = False)#!!!!!!!!!!!!!!!!!!! + -> -    fait reculer le robot pout le sortir de la zone rouge autour de la balle

    def sortie_de_terrain():
        if c.blue1.position[0]>1:
            c.blue1.goto((c.blue1.position[0]-0.1 ,c.blue1.position[1]), wait = False)
        elif c.blue1.position[0] < -1:
            c.blue1.goto((c.blue1.position[0]+0.1 ,c.blue1.position[1]), wait = False)
        elif c.blue1.position[1] < -1:
            c.blue1.goto((c.blue1.position[0], c.blue1.position[1]+0.1), wait = False)
        elif c.blue1.position[1] > 1:
            c.blue1.goto((c.blue1.position[0], c.blue1.position[1]-0.1), wait = False)
    ...
    def attaquant():
        ...
        cos = distance2(y1)
        while d >= 0.12:  # si d <0.11 le robot est a distance pour tirer
    ...
    def positionnement1(y1,cos, y_Axe):
        if y1 >= c.ball[1]:
            #c.blue1.goto((c.ball[0] - 0.2, y_Axe, 0 - cos), wait = False)  # le robot se positionne sur la droite (y)
            c.blue1.goto((c.ball[0]-0.05, c.ball[1]-0.03, 0 - cos), wait = False)  # le robot avance a la balle
        else:
            #c.blue1.goto((c.ball[0] - 0.2, y_Axe, 0 + cos), wait = False)  # le robot se positionne sur la droite (y)
            c.blue1.goto((c.ball[0]-0.05, c.ball[1]+0.03, 0 + cos), wait = False) #le robot avance a la balle
    ...
    def gardien():
        ...
        if -0.3<= y_goal <= 0.3:
            print(y_goal)
            c.blue2.goto((-0.8, y_goal, 0.), wait = False)
        if abs(xb2 - xb) <= 0.25 and abs(yb2 - yb) <= 0.25:
            kick2(t_ref)
    ...
    def recule(y1,cos):
        if y1>=c.ball[1] :
            c.blue1.goto((c.blue1.position[0]-0.2,c.blue1.position[1], 0), wait = False) #!!!!!!!!!!!!!!!!!!!! + -> -   #fait reculer le robot pout le sortir de la zone rouge autour de la balle
        else :
            c.blue1.goto((c.blue1.position[0]-0.2 ,c.blue1.position[1], 0), wait = False)#!!!!!!!!!!!!!!!!!!! + -> -    fait reculer le robot pout le sortir de la zone rouge autour de la balle
